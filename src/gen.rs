...
pub fn init(project: &str, rel: &str) -> Result<()> {
    let project = Path::new(project);
    let dir = module_dir(project, rel);
    std::fs::create_dir_all(&dir)?;

    std::fs::write(dir.join("get_cpm.cmake"), GET_CPM_DEFAULT_CMAKE)?;
    std::fs::write(dir.join("3rdparty.cmake"), ENGINE_CMAKE)?;
    ...
    std::fs::write(dir.join("deps.toml"), toml)?;

    println!("initialized cpm module: {}", dir.display());
    println!();
