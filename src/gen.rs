...
pub fn init(project: &str, rel: &str, scripts: &str, patches: &str, force: bool) -> Result<()> {
    ...
    let toml = format!(
         "# cpm project manifest. Layers: global pantry ($CPM_HOME/deps.toml) is the base,\n\
          # this file overlays it (more local wins). After editing (from project root): cpm generate\n\
         project      = \"{name}\"\n\
...
pub fn generate(project: &str, rel: &str) -> Result<()> {
    ...
    generate_with_pantry(project, rel, &pantry)
}

pub fn generate_with_pantry(project: &str, rel: &str, pantry: &deps::Registry) -> Result<()> {
    let project = Path::new(project);
    let dir = module_dir(project, rel);
    ...
    let mut generated: Vec<(String, String)> = Vec::new(); // (package, toml-key)
    ...
    for key in &keys {
        ...
        // Resolve the CMake package name (the find_package() / Find<Name>.cmake
        // name). Priority: an explicit `package` override that genuinely differs
        // from the key; else the pantry's canonical name; else the key itself
        // (synthetic/no_source deps with no pantry entry). A `package` that is
        // byte-identical to the key is treated as no override — it's a redundant
        // artifact of older `cpm requires add` (which wrote `package = <key>`),
        // so the pantry canonical name wins and `[dep.boost]` yields `FindBoost.cmake`.
        let pkg = match spec.package.as_deref() {
            Some(p) if p != key => p.to_string(),
            _ => base.map(|b| b.name.clone()).unwrap_or_else(|| key.clone()),
        };
        ...
        generated.push((pkg.clone(), key.clone()));
    }
    ...
    // Prune stale / wrong-cased generated Find<Name>.cmake, then write fresh.
    // Stale = a dep no longer present; wrong-cased = a current dep whose on-disk
    // filename case differs from the canonical package (e.g. Findboost.cmake left
    // over once the package resolves to Boost). Removing first matters on NTFS,
    // which is case-insensitive but case-preserving: an in-place overwrite keeps
    // the OLD filename. Only touch files `cpm generate` produced (header marker)
    // or wrong-cased variants of current deps — never hand-written modules.
    if let Ok(entries) = std::fs::read_dir(&dir) {
        for e in entries.flatten() {
            let fname = e.file_name().to_string_lossy().into_owned();
            let lower = fname.to_ascii_lowercase();
            if !(lower.starts_with("find") && lower.ends_with(".cmake") && fname.len() > 10) {
                continue;
            }
            let stem = &fname[4..fname.len() - 6]; // between "Find" and ".cmake"
            if generated.iter().any(|(p, _)| p == stem) {
                continue; // current dep, correct case — overwritten below
            }
            if generated.iter().any(|(p, _)| p.eq_ignore_ascii_case(stem)) {
                // wrong case of a current dep — drop so it's recreated canonical
                let _ = std::fs::remove_file(e.path());
            } else if let Ok(c) = std::fs::read_to_string(e.path()) {
                // stale dep — remove only if cpm-generated (preserve hand-written)
                if c.contains("generated by `cpm generate`") {
                    let _ = std::fs::remove_file(e.path());
                }
            }
        }
    ...
    // Find<Package>.cmake per dep. remove_file before write so the canonical case
    // takes effect on disk (NTFS keeps the old case on an in-place overwrite).
    for (pkg, fkey) in &generated {
        let content = format!(
        ...
        let path = dir.join(format!("Find{pkg}.cmake"));
        let _ = std::fs::remove_file(&path);
        std::fs::write(&path, content)?;
    }

    println!("generated {} dep(s) -> {}", generated.len(), dir.display());
    for (pkg, _) in &generated {
...
mod tests {
    ...
    /// Golden: the static registrations `generate` emits for a representative
    /// manifest + pantry. Catches regressions in version resolution, package
    /// defaulting, options (incl. DOWNLOAD_ONLY), and tier synthesis.
    #[test]
    fn generate_registrations_snapshot() {
        use crate::deps::{Dep, Registry};
        let reg = Registry {
            meta: Default::default(),
            deps: vec![
                Dep {
                    name: "fmt".into(),
                    url: Some("https://github.com/fmtlib/fmt.git".into()),
                    tag: Some("10.2.1".into()),
                    archive: "fmt-10.2.1.zip".into(),
                    version: Some("10.2.1".into()),
                ...
                Dep {
                    name: "hdr".into(),
                    url: Some("https://github.com/x/hdr.git".into()),
                    tag: Some("1.0.0".into()),
                    archive: "hdr-1.0.0.zip".into(),
                    version: Some("1.0.0".into()),
                    sha256: Some("cafef00d".into()),
                    added_at: None,
                    submodules: None,
                },
            ],
        ...
        let tmp = std::env::temp_dir().join(format!("cpm_gen_test_{}", std::process::id()));
        let m = tmp.join("m");
        std::fs::create_dir_all(&m).unwrap();
        std::fs::write(
            m.join("deps.toml"),
            "project = \"t\"\nnetwork_when = \"CPM_DOWNLOAD\"\ndeps = [\"fmt\"]\n\
             [dep.hdr]\npackage = \"HDR\"\ndownload_only = true\n",
        )
        .unwrap();

        generate_with_pantry(tmp.to_str().unwrap(), "m", &reg).unwrap();
        let out = std::fs::read_to_string(m.join("3rdparty.cmake")).unwrap();

        let expected = "\
\n# ===== registrations (generated by `cpm generate`) =====\n\
cpm_declare_package(KEY fmt PACKAGE fmt VERSION 10.2.1 OPTIONS OVERRIDE_FIND_PACKAGE)\n\
cpm_declare_fallback(KEY fmt TIER git WHEN CPM_DOWNLOAD GIT https://github.com/fmtlib/fmt.git TAG 10.2.1 OPTIONS GIT_SHALLOW ON)\n\
cpm_declare_fallback(KEY fmt TIER loc URL ${CPM_PRELOAD}/fmt-10.2.1.zip HASH SHA256=deadbeef)\n\
cpm_declare_package(KEY hdr PACKAGE HDR VERSION 1.0.0 OPTIONS OVERRIDE_FIND_PACKAGE DOWNLOAD_ONLY)\n\
cpm_declare_fallback(KEY hdr TIER git WHEN CPM_DOWNLOAD GIT https://github.com/x/hdr.git TAG 1.0.0 OPTIONS GIT_SHALLOW ON)\n\
cpm_declare_fallback(KEY hdr TIER loc URL ${CPM_PRELOAD}/hdr-1.0.0.zip HASH SHA256=cafef00d)\n";
        assert!(
            out.ends_with(expected),
            "registrations body mismatch.\n--- expected tail ---\n{expected}\n--- actual tail ---\n{}",
            &out[out.len().saturating_sub(expected.len() + 400)..]
        );

        // Find<Package>.cmake emitted per dep, named after the package.
        assert!(m.join("Findfmt.cmake").exists(), "Findfmt.cmake missing");
        assert!(m.join("FindHDR.cmake").exists(), "FindHDR.cmake missing");

        let _ = std::fs::remove_dir_all(&tmp);
    }

    /// The package name (find_package / Find<Name>.cmake) follows the pantry's
    /// canonical name, so a lowercase `[dep.boost]` yields `FindBoost.cmake` once
    /// the pantry entry is canonical (`cpm add Boost` / `cpm rename boost Boost`).
    /// A redundant `package = <key>` artifact (written by older `cpm requires
    /// add`) is ignored, and a stale wrong-case Find file is pruned.
    #[test]
    fn generate_canonicalizes_package_from_pantry() {
        use crate::deps::{Dep, Registry};
        let reg = Registry {
            meta: Default::default(),
            deps: vec![Dep {
                name: "Boost".into(),
                url: Some("https://github.com/boostorg/boost.git".into()),
                tag: Some("boost-1.90.0".into()),
                archive: "boost-1.90.0.zip".into(),
        ...
        let tmp = std::env::temp_dir().join(format!("cpm_gen_canon_{}", std::process::id()));
        let m = tmp.join("m");
        std::fs::create_dir_all(&m).unwrap();
        // seed a stale wrong-case Find file (should be pruned/renamed away)
        std::fs::write(
            m.join("Findboost.cmake"),
            "# Findboost.cmake — generated by `cpm generate`. Do not edit by hand.\nstale\n",
        )
        .unwrap();
        // lowercase key with a redundant `package = "boost"` artifact
        std::fs::write(
            m.join("deps.toml"),
            "project = \"t\"\nnetwork_when = \"CPM_DOWNLOAD\"\n[dep.boost]\npackage = \"boost\"\n",
        )
        ...
        // KEY stays the project handle `boost`; PACKAGE is the canonical `Boost`
        // (the redundant package artifact did not pin it to lowercase).
        assert!(
            out.contains("cpm_declare_package(KEY boost PACKAGE Boost VERSION 1.90.0"),
            "expected KEY boost PACKAGE Boost in:\n{out}"
        );
        // Find file uses the canonical case and resolves via the key.
        assert!(m.join("FindBoost.cmake").exists(), "FindBoost.cmake missing");
        let find = std::fs::read_to_string(m.join("FindBoost.cmake")).unwrap();
        assert!(
            find.contains("cpm_resolve_fallback(\"boost\")"),
            "FindBoost.cmake must resolve via the toml key 'boost'"
        );
        assert!(!find.contains("stale"), "stale Findboost.cmake content should be gone");
