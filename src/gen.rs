...
pub fn init(project: &str, rel: &str, scripts: &str, patches: &str) -> Result<()> {
    ...
    let cpm = format!(
        "# .cpm — generated by `cpm init`. Tool orientation; CMake does NOT read this.\n\
         # Regenerate by re-running `cpm init` with updated --dir/--scripts/--patches.\n\
         [paths]\n\
    ...
    std::fs::write(project.join(".cpm"), cpm)?;
    ...
    println!("wrote project root:     {}", project.join(".cpm").display());
    println!();
